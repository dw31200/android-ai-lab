...
| assistant-contract-designer | request/response/error 계약 설계 | `contract.md`, result shape, error code |
| assistant-scenario-curator | 실제 사용 시나리오 작성 | `scenarios.md`, 샘플 입력, 검증 기준 |
| assistant-qa-validator | 계약, 시나리오, 스킬 문서 정합성 검증 | QA report, regression checklist |

## Workflow

1. product architect가 기능 범위와 위험한 자동 액션 제한을 정한다.
2. contract designer가 기능별 구조화 출력과 실패 계약을 만든다.
3. scenario curator가 정상, 경계, 실패 시나리오를 Markdown으로 작성한다.
4. QA validator가 scenario ID, 계약 shape, assertion, timezone, privacy 기준을 검증한다.
5. Blocker와 Major가 0이 되면 구현 또는 SDK 연결 단계로 넘어간다.

## Communication Rules

- 제품 범위 변경은 계약과 시나리오를 함께 재검토한다.
- 시나리오 변경은 검증 기준과 함께 바꾼다.
- 계약 변경은 기존 시나리오 regression 결과를 확인한 뒤 반영한다.
...
| 시나리오가 계약과 맞지 않음 | assistant-contract-designer | 계약 또는 시나리오 shape 수정 |
