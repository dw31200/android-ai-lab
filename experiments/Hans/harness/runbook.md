...
3. `skills/`에서 해당 작업에 맞는 스킬 문서를 확인한다.
...
- `locale`: `ko-KR`
- `timezone`: `Asia/Seoul`
- 날짜 해석 기준: 테스트 실행 시각 또는 시나리오에 명시된 기준 시각

## 2. 실행

각 시나리오의 샘플 입력을 SDK 또는 모델 호출 계층에 전달한다.
...
1. 기능 추가 또는 계약 변경 전 현재 시나리오 전체를 실행한다.
2. 변경 후 같은 시나리오를 다시 실행한다.
3. 실패가 새로 생기면 regression으로 기록한다.
4. 의도된 계약 변경이면 `contract.md`, `scenarios.md`, 관련 skill 문서를 같은 커밋에서 갱신한다.
...
- API 키, 토큰, 실제 연락처, 실제 계정 정보는 로그나 시나리오에 남기지 않는다.
